...
from gptdb_hub_sql.configs.config import INPUT_PROMPT, INSTRUCTION_PROMPT
...
class SqlDataProcessor(object):
    def __init__(
        ...
        if data_folder is None:
            self.data_folder = os.path.join(ROOT_PATH, "gptdb_hub_sql/data")
            print(
                "The user do not provide exact data folder, we take 'gptdb_hub_sql/data' as the default folder"
            )
        else:
...
if __name__ == "__main__":
    data_folder = os.path.join(ROOT_PATH, "gptdb_hub_sql/data")
    data_info = [
        {
            "data_source": "spider",
