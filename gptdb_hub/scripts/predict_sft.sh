...
pred_log="gptdb_hub/output/logs/pred_test_${current_date}.log"
start_time=$(date +%s)
echo " Pred Start time: $(date -d @$start_time +'%Y-%m-%d %H:%M:%S')" >>${pred_log}

CUDA_VISIBLE_DEVICES=0,1  python gptdb_hub/predict/predict.py \
    --model_name_or_path Your_download_CodeLlama-13b-Instruct-hf_path \
    --template llama2 \
    --finetuning_type lora \
    --predicted_input_filename gptdb_hub/data/example_text2sql_dev.json \
    --checkpoint_dir gptdb_hub/output/adapter/CodeLlama-13b-sql-lora \
    --predicted_out_filename gptdb_hub/output/pred/pred_codellama13b.sql >> ${pred_log}
...
#     --quantization_bit 4 \
#     --finetuning_type lora \
#     --checkpoint_dir gptdb_hub/output/adapter/baichuan2-13b-qlora 
...
#     --template llama2 \
#     --finetuning_type lora \
#     --checkpoint_dir gptdb_hub/output/adapter/code_llama_7b-qlora